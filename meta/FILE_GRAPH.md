...
>
> 更新: 2026-06-11 | 维护规则：新增/移动文件或目录时同步更新本图谱。
...
│   └── ai-workflow_AI协作开发流程/  #   方法论 v2.0（长文拆目录，见 ADR 0005）
│       ├── 01-流程全景.md            #   两类任务（长程 / 思考）+ 触发条件
│       ├── 02-模块分类.md            #   工程模块 vs 基础+功能模块
│       ├── 03-设计规范.md            #   设计文件夹 7 件套
│       ├── 04-长程开发.md            #   AI决策→预先计划→验收报告 + TDD
│       ├── 05-思考设计.md            #   讨论 Agent 五步法
│       ├── 06-端到端流程.md          #   PRD→BDD→…→部署→交付
│       ├── 07-验收交付.md            #   三级验收 + 交付物 + 文件放置速查
│       ├── 08-汇报收束.md            #   功能批次闸门：整理/测试/审计/验证 + 汇报
│       └── 09-部署规范.md            #   统一服务器（max）部署 + 监控
...
│   │   └── design/                   #     复杂功能设计文档（每个设计文件夹 7 件套：README/简报/设计/实现计划/阅读笔记/deliverable + 2 个 HTML）
│   │       ├── README-设计文档说明.md #       设计文档索引与模板
│   │       ├── 设计提案-流程强制化方案.md   #     V2.1 流程强制化方案总纲
│   │       ├── 设计提案-约束与模板强化方案-v2.3.md  #  V2.3 #48-#53 总纲（两层强制模型 + A-E 实施）
│   │       ├── 设计提案-图谱与代码理解规范-v2.md
│   │       ├── M1-conventions-规范正文/      #     流程强制化方案拆分（M1-M8，每个设计文件夹 7 件套）
│   │       ├── M2-docs-文档资产/
│   │       ├── M3-src-示例代码/
│   │       ├── M4-scripts-工具脚本/
│   │       ├── M5-worklogs-工作日志/
...
│   └── 豁免清单.md                    #   豁免登记账（V2.3 #51）：[skip-*] 标记目录 + 使用记录，check_exemption_log.py 强制
│
...
│     → docs/plan/design/<设计名>/  ← 每个设计 1 个文件夹，含 7 件套（README/简报/设计/实现计划/阅读笔记/deliverable + 2 HTML）
│       或 docs/plan/design/设计文档-<功能名>.md（单文件轻量场景）
│
├─ 进度汇报（功能点完成 / 收束报告）？
