...
│   │   └── design/                   #     复杂功能设计文档（五段式）+ 红蓝整理 6 份提案（设计提案-整理大类一~六）
│   │       ├── README-设计文档说明.md #       设计文档索引与模板
│   │       └── 设计提案-图谱与代码理解规范-v2.md
│   ├── reports/                      #   进度汇报（功能点/周期/阶段，从模板生成）
│   ├── research/                     #   调研文档
│   ├── 历史文件/                      #   历史归档（重构笔记 + v1.0 历史摘要 + 归档索引；v1.0 正文已压缩，仅存 git 历史）
...
5. **堆积收敛**（2026-08-13 红队 R-05 调和后定稿）：worklogs **原路径不动**（守 §四命名规约⑨「存量不重命名」）；旧年日志以「复制 + 索引」方式收敛——顶层保留当年，旧年条目登记入 `docs/历史文件/README.md` 归档索引即可检索；reports 按版本收敛合并；research 完工后封存（对应方案 ⑪）。
