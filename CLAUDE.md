...
纯文档项目（Markdown 规范 + Python 示例），Git 管理。本身是一套可一键初始化到新项目的开发规范模板。目标 49 个功能点，当前 48 个已交付；#49 本地闭环完成，待 GitHub 平台保护规则回读。
...
- **进度**：48/49 🔄（#47/#48 已完成；#49 待 GitHub `master` 保护规则回读）
- **规范版本**：01-06 为 **v2.0**，08 为 **v3.1**；治理基线为 **V2.1.0**，`src/<维度>/` 配有对应可运行配置
- **ADR**：7 个（`worklogs/decisions/0001-0007`）
- **收束节点**：v0.1–v1.5 共 15 个历史节点已执行；其中 **v0.1 已人审计签核**，v0.2–v1.5（14 个）人审计仍 ⏳ 待签核；本轮 P0/P1 收束实施中（见 `STATUS.md`）
