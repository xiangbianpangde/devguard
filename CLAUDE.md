...
- **进度**：55/55（#50-#52 ECC 全项目对标与一键初始化 2.0 已收束；#53 报告格式入族、#55 单命令跨平台安装器已交付；#54 merged_report 存量迁移 2026-08-13 收官——55 个功能点全部完成，V2.5.0 已发布）
- **规范版本**：01-06 为 **v2.0**，08 为 **v3.1**；治理基线为 **V2.4.0**，`src/<维度>/` 配有对应可运行配置
- **ADR**：9 个（`worklogs/decisions/0001-0009`，0009=对抗式验证固化）
- **收束节点**：v0.1–v2.3 共 18 个节点已执行；其中 **v0.1 已人审计签核**，v0.2–v2.3（17 个）人审计仍 ⏳ 待签核；本轮机器收束通过（见 `STATUS.md`）
