...
本模板族已注册 `doc-template: final-report`，由 `scripts/check_html_artifact.py` 硬校验（不再是 untyped WARN 地位）。

- **类型声明**：`<meta name="doc-template" content="final-report">`（template.html / demo.html 已携带；复制后**必须保留**）
- **结构锚点**（缺一即 FAIL）：`class="hero"` + `class="kpi-row"` + `class="toc"`（TOC 锚点导航，通用 `<nav>` 也必查）+ `class="verdict"` + **≥2 个** `class="mermaid"` 图块
- **校验命令**：
  - 单文件（随 commit-msg 自动）：`python scripts/check_html_artifact.py <commit_msg_file>`
  - 全仓审计：`python scripts/check_html_artifact.py --all`
- **存量**：未声明类型的高密度风存量报告仅 WARN 不拦，逐个迁移或豁免见功能点 #54（merged_report 等不在本轮打标）。
