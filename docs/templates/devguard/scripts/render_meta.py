...
# -*- coding: utf-8 -*-
"""
 模板：render_meta.py（_meta.yaml 渲染脚本）
=========================================================
复制为:  {新项目仓库}/scripts/render_meta.py
配套:    复制 conventions/_meta.yaml 模板（在同一 devguard 模板目录）
依赖:    pip install pyyaml

个性化改造:
    本脚本本身不需要个性化，直接复制即用。
    个性化通过改 conventions/_meta.yaml 实现，本脚本读取它来渲染下游产物。

最小时（v0.1）: 只支持 pre-commit-config 渲染和校验
扩展（v0.2+）:  会加 convention-grade 渲染（把分级标签投射到每篇规范顶部）
"""
...
def render_pre_commit_config(meta: dict) -> str:
    ...
    lines: list[str] = [
        ...
        "# ============================================================",
        "# ️  渲染产物 — 由 scripts/render_meta.py 从 conventions/_meta.yaml 自动生成",
