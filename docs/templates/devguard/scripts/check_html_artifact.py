...
# R5-27（2026-08-14 红队第二轮）：final-report 12 节固定骨架锚点（§00-§12）。
# 依据 docs/templates/devguard/final-report-template/README.md「章节骨架（12 节固定）」；
# demo.html 以 <section id="s1">~<section id="s12"> + <section id="hero"> 承载。
# 删除整节（section 块）→ 锚点缺失 → FAIL（此前仅查 4 个 class 锚点，删节不报）。
FINAL_REPORT_SECTION_ANCHORS = ["hero"] + [f"s{i}" for i in range(1, 13)]
...
def final_report_anchor_errors(content: str) -> list[str]:
    ...
    校验：4 个 class 锚点 + ≥2 mermaid + 12 节 section 骨架（R5-27 新增）。
    """
    missing: list[str] = []
    ...
    # R5-27：12 节固定骨架（§00 hero + §01-§12），缺任一节即 FAIL
    present_ids = set(SECTION_ID.findall(content))
    for anchor in FINAL_REPORT_SECTION_ANCHORS:
        if anchor not in present_ids:
            missing.append(f'section id="{anchor}"（12 节骨架）')
    return missing
...
def exempted_files(root: Path) -> set[str]:
    """读取 meta/豁免清单.md §三中 [skip-html] 登记的文件路径（--all 审计豁免）。

    R5-27（2026-08-14）：--all 全仓审计此前无豁免机制，存量 8 节骨架的
    final-report（07-17 验收报告）在新 12 节契约下会 FAIL；豁免须在
    meta/豁免清单.md 登记（C6 可审计），此处只读登记账。
    """
    log = root / "meta" / "豁免清单.md"
    if not log.is_file():
        return set()
    ...
    for line in log.read_text(encoding="utf-8").splitlines():
        ...
        if len(cells) < 7:
            continue
        scope, hook, marker = cells[1], cells[2], cells[3]
        if "[skip-html]" not in marker or "check_html_artifact" not in hook:
            continue
        # 范围可能含描述性文字（如「（final-report 类型，8 节旧骨架）」）；
        # 用正则提取所有 .html 路径片段（R5-27：中英文标点混合时逐字拆分不可靠）
        for part in re.findall(r"[\w/\-.（）()、，, ]+?\.html", scope):
...
def audit_all() -> int:
    ...
    exempted = exempted_files(REPO_ROOT)
    errors: list[str] = []
    warnings: list[str] = []
    n = 0
    for prefix in SCOPE_PREFIXES:
        ...
        for fp in sorted(base.rglob("*.html")):
            ...
            if rel in exempted:
                warnings.append(f"{rel}: [skip-html] 豁免（登记于 meta/豁免清单.md）")
                continue
            n += 1
