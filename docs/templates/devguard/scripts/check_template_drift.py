...
# 脚手架载荷镜像（真源 ↔ scaffold 载荷，逐字节一致才算无漂移）
...
def check_scaffold_mirrors(root: Path) -> list[str]:
    """对比 scaffold 载荷关键文件与真源，返回漂移描述列表"""
    template_dir = root / "docs" / "templates" / "devguard"
    errors: list[str] = []
    if not template_dir.is_dir():
        # 新初始化项目无 devguard 模板族（scaffold 载荷不含 templates 镜像），跳过
        return []
    for scaffold_rel, live_rel in SCAFFOLD_MIRRORS:
        scaffold_path = template_dir / scaffold_rel
        live_path = root / live_rel
        if not scaffold_path.is_file():
            errors.append(f"脚手架镜像缺失: {scaffold_path.relative_to(root)}")
            continue
        if not live_path.is_file():
            errors.append(f"真源缺失: {live_rel}")
            continue
...
def main() -> int:
    ...
    print(f"OK 模板漂移检测通过（{n_files} 类入口文件 + 脚本镜像 + 脚手架镜像，一致）")
    return 0
