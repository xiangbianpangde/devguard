...
# R5-12（2026-08-14 红队第六轮）：节点编号合法性上限——
# 防「节点编号放大绕过」（nodes=58→1000 使 staged #59 不越界）。
# 上限 = 开发清单最大功能点编号 + 1（预留收尾节点），绝对上限 500。
MAX_NODE_ABS = 500
...
def parse_marker(text: str) -> tuple[list[int], int] | None:
    ...
    if not nodes or any(node <= 0 for node in nodes) or last < 0:
        return None
    if any(node > MAX_NODE_ABS for node in nodes) or last > MAX_NODE_ABS:
        return None  # 编号放大 = 标记非法（fail-closed）
    return nodes, last
