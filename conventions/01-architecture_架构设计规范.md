...
```
表现层 (UI/API) → 应用层 (用例编排) → 基础设施层 (DB/缓存/外部服务) → 领域层 (业务规则)
依赖方向单向向下；领域层定义接口、基础设施层实现接口（依赖倒置，infrastructure 依赖 domain）。
```
...
| 方向 | 链接 |
|------|------|
| 细化自 | [ai-workflow 04-长程开发（预先计划）](ai-workflow_AI协作开发流程/04-长程开发.md) |
