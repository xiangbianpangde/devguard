...
> **本规范是 [ai-workflow 04-长程开发](ai-workflow_AI协作开发流程/04-长程开发.md)（迭代开发）的细化**，把工作流里两个一句话带过的环节展开成可执行标准：
> - 细化 **§四 TDD 实现环节** —— 写代码时必须满足的硬约束（命名 / 结构 / 错误处理 / 安全 / 日志）。
> - 细化 **§四 审查 diff 环节** —— 审 `git diff` 时逐条核对的清单。本规范 §六「检查清单」就是该环节审查清单的展开版。
>
> **档位裁剪**（任务分类见 [01-流程全景](ai-workflow_AI协作开发流程/01-流程全景.md)）：轻量项目只守 §一「红线」；标准 / 团队项目守全部。
> 更新: 2026-06-11
...
可操作的关键在于——规则不靠自觉，靠 pre-commit 和 CI 拦截。新项目在 [06-端到端流程 §分层启动·脚手架层](ai-workflow_AI协作开发流程/06-端到端流程.md) 时一并落地。
...
| 方向 | 链接 |
|------|------|
| 细化自 | [ai-workflow 04-长程开发 §四 实现 / 审查 diff](ai-workflow_AI协作开发流程/04-长程开发.md) |
