...
受管范围（V2.3 #54 泛化为全仓文档）= 所有 git 跟踪的 .md，排除 docs/templates/ /
worklogs/ / .github/ / CHANGELOG.md；在范围文件必须携带 `> 更新: YYYY-MM-DD` 标签，
...
# 受管范围（V2.3 #54 泛化）：所有 git 跟踪的文档 .md，排除——
#   docs/templates/（模板）/ worklogs/（历史日志）/ .github/（PR/Issue 模板）/ CHANGELOG.md
# 范围决策见 meta/豁免清单.md 与设计 §阶段B。
EXCLUDE_PREFIXES = ("docs/templates/", "worklogs/", ".github/")
EXCLUDE_EXACT = {"CHANGELOG.md"}


def in_scope(rel: str) -> bool:
    """该相对路径是否属于"必须携带并刷新「更新」标签"的文档。"""
    rel = rel.replace("\\", "/")
    if not rel.endswith(".md"):
        return False
    if rel.startswith(EXCLUDE_PREFIXES):
        return False
    return rel not in EXCLUDE_EXACT
